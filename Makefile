...
IMG_BASE ?= kdop-operator
IMG ?= $(IMG_BASE):$(shell cat VERSION 2>/dev/null || echo latest)
...
KIND_CLUSTER ?= kdop-operator-test-e2e
...
.PHONY: bump
bump: ## Bump patch version in VERSION file
...
.PHONY: dist
dist: manifests ## Generate install/install.yaml (CRDs + RBAC + gateway + manager)
	...
	@if [ -f config/manager/namespace.yaml ]; then cat config/manager/namespace.yaml >> install/install.yaml; echo "---" >> install/install.yaml; fi
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name kdop-operator-builder
	$(CONTAINER_TOOL) buildx use kdop-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm kdop-operator-builder
	rm Dockerfile.cross

.PHONY: build-installer
build-installer: manifests generate kustomize ## Generate a consolidated YAML with CRDs and deployment.
	mkdir -p dist
	"$(KUSTOMIZE)" build config/default | sed "s|IMG_PLACEHOLDER|${IMG}|g" > dist/install.yaml
...
deploy: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	"$(KUSTOMIZE)" build config/default | sed "s|IMG_PLACEHOLDER|${IMG}|g" | "$(KUBECTL)" apply -f -
